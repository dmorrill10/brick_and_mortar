...
describe Mortar::Git do
  describe 'given git ssh brick configuration' do
    git_ssh_brick_configruation = MockBrickConfiguration.new(
      'mock_brick',
      '0.0.1',
      MockBrickLocation.new('git', 'git@github.com:dmorrill10/mortar.git')
    )

    it 'downloads the brick with git ssh' do
      Dir.mktmpdir do |dir|
        Dir.chdir dir do |d|
          Mortar::Git.clone_repo(git_ssh_brick_configruation.location.url, git_ssh_brick_configruation.name)
  ...
  describe 'given git https brick configuration' do
    git_https_brick_configruation = MockBrickConfiguration.new(
      'mock_brick',
      '0.0.1',
      MockBrickLocation.new('git', 'https://github.com/dmorrill10/mortar.git')
    )

    it 'downloads the brick with git https' do
      Dir.mktmpdir do |dir|
        Dir.chdir dir do |d|
          Mortar::Git.clone_repo(git_https_brick_configruation.location.url, git_https_brick_configruation.name)
